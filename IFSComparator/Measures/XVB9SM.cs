...
namespace IFSComparator.Measures
{
    ...
    class XVB9SM : IFSMeasure
    {
        ...
        public XVB9SM(Double alpha, Double delta, int k)
        {
            _alpha = alpha;
            _delta = delta;
            _isDirty = true;
        ...
        private Dictionary<int, int> SortByDescending(Models.IFS R, Models.IFSElementComparer.FocusedCriterion focusedCriterion)
        {
            Dictionary<int, int> ret = new Dictionary<int, int>();
            IOrderedEnumerable<KeyValuePair<Int32, Models.IFSElement>> query = R.OrderByDescending<KeyValuePair<Int32, Models.IFSElement>, Models.IFSElement>(se => se.Value, new Models.IFSElementComparer(focusedCriterion));
        ...
        private Double ApproximateCDF(Models.IFS P, Models.IFS Q, int k, double delta, double alpha, Models.IFSElementComparer.FocusedCriterion focusedCriterion)
        {
            Double ret = 0;

            Dictionary<int, int> sortedP = SortByDescending(P, focusedCriterion);
            int n = sortedP.Count;
            int j = 0;
            ...
            foreach (KeyValuePair<int, int> pair in sortedP)
            {
                ...
                if (Math.Abs(spotDiffSwap) < Math.Abs(spotDiff)) //More aligned to the opposite judgment
                {
                    if (P[pair.Key].Membership > P[pair.Key].Nonmembership)
                    { // Membership is more influent
                        if ((P[pair.Key].Membership + alpha * P[pair.Key].Hesitation) > delta)
                            oppositeRatio = (Q[pair.Key].Nonmembership + alpha * Q[pair.Key].Hesitation) / (P[pair.Key].Membership + alpha * P[pair.Key].Hesitation);
                    }
                    else if (P[pair.Key].Membership < P[pair.Key].Nonmembership)
                    { // non-membership is more influent
                        if ((P[pair.Key].Nonmembership + (1 - alpha) * P[pair.Key].Hesitation) > delta)
                            oppositeRatio = (Q[pair.Key].Membership + (1 - alpha) * Q[pair.Key].Hesitation) / (P[pair.Key].Nonmembership + (1 - alpha) * P[pair.Key].Hesitation); //XVB8b -> easy to explain
                    }
                ...
                else if (Math.Abs(spotDiffSwap) > Math.Abs(spotDiff)) //More aligned to the given judgment
                {
                    if (P[pair.Key].Membership > P[pair.Key].Nonmembership)
                    { // Membership is more influent
                        if ((P[pair.Key].Membership + alpha * P[pair.Key].Hesitation) > delta)
                            supportRatio = (Q[pair.Key].Membership + alpha * Q[pair.Key].Hesitation) / (P[pair.Key].Membership + alpha * P[pair.Key].Hesitation);
                    }
                    else if (P[pair.Key].Membership < P[pair.Key].Nonmembership)
                    { // non-membership is more influent
                        if ((P[pair.Key].Nonmembership + (1 - alpha) * P[pair.Key].Hesitation) > delta)
                            supportRatio = (Q[pair.Key].Nonmembership + (1 - alpha) * Q[pair.Key].Hesitation) / (P[pair.Key].Nonmembership + (1 - alpha) * P[pair.Key].Hesitation);
                    }
        ...
        public Result GetSimilarityExtended(Models.IFS P, Models.IFS Q)
        {
            ...
            double cdfMembership = ApproximateCDF(P, Q, _k, _delta, _alpha, Models.IFSElementComparer.FocusedCriterion.Membership);
            //double cdfNonMembership = ApproximateCDF(P, Q, _k, _delta, _alpha, Models.IFSElementComparer.FocusedCriterion.NonMembership);

            _result = new Result();
            _result.GrossLevel = grossLevel;
            _result.CDFMembership = cdfMembership;
            //_result.CDFNonMembership = cdfNonMembership;
            //_result.CDF = Math.Min(cdfMembership, cdfNonMembership);
            _result.CDF = cdfMembership;
            //_result.CDF = cdfNonMembership;
