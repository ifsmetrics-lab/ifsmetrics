...
namespace IFSComparator.Measures
{
    ...
    class XVB12SM : IFSMeasure
    {
        ...
        public XVB12SM(Double alpha, Double beta, Double delta, int k)
        {
            _alpha = alpha;
            _delta = delta;
            _isDirty = true;
        ...
        private Double GetMembershipCDFByOppositeVsSupportRatio(Models.IFS P, Models.IFS Q, int k, double delta, double alpha, double beta)
        {
            Double ret = 0;
            ...
            foreach (KeyValuePair<int, int> pair in sortedP)
            {
                ...
                if (Math.Abs(spotDiffSwap) < Math.Abs(spotDiff)) //More opposite
                {
                    if ((P[pair.Key].Membership + alpha * P[pair.Key].Hesitation) > delta)
                        oppositeRatio0 = (Q[pair.Key].Nonmembership + beta * Q[pair.Key].Hesitation) / (P[pair.Key].Membership + alpha * P[pair.Key].Hesitation);

                    if ((P[pair.Key].Nonmembership + beta * P[pair.Key].Hesitation) > delta)
                        oppositeRatio1 = (Q[pair.Key].Membership + alpha * Q[pair.Key].Hesitation) / (P[pair.Key].Nonmembership + beta * P[pair.Key].Hesitation); //XVB8b -> easy to explain
                ...
                else if (Math.Abs(spotDiffSwap) > Math.Abs(spotDiff)) //More supportive
                {
                    ...
                    if ((P[pair.Key].Nonmembership + beta* P[pair.Key].Hesitation) > delta)
                        supportRatio1 = (Q[pair.Key].Nonmembership + beta * Q[pair.Key].Hesitation) / (P[pair.Key].Nonmembership + beta * P[pair.Key].Hesitation);
        ...
        private Double GetNonMembershipCDFByOppositeVsSupportRatio(Models.IFS P, Models.IFS Q, int k, double delta, double alpha, double beta)
        {
            Double ret = 0;
        ...
        public Result GetSimilarityExtended(Models.IFS P, Models.IFS Q)
        {
            ...
            double cdf = GetMembershipCDFByOppositeVsSupportRatio(P, Q, _k, _delta, _alpha, _beta);
            double nonMcdf = GetNonMembershipCDFByOppositeVsSupportRatio(P, Q, _k, _delta, _alpha, _beta);
        ...
        {          
            return GetNonMembershipCDFByOppositeVsSupportRatio(P, Q, _k, _delta, _alpha, _beta); 
        }

        public double GetMembershipCDF(Models.IFS P, Models.IFS Q)
        {
            return GetMembershipCDFByOppositeVsSupportRatio(P, Q, _k, _delta, _alpha, _beta);
        }
