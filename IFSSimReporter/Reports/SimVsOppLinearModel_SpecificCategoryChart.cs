...
{
	public class SimVsOppLinearModel_SpecificCategoryChart:GNUPlotBasedChart
	{
		...
		public SimVsOppLinearModel_SpecificCategoryChart(
			String category,
			String measure,
		...
		public string Build()
		{
			...
			String reportPath = String.Format ("{0}{1}_{2}.LM{3}", _prms.ReportsDir, _category, _measure.Replace('.','_'), ext);
			_log.WriteLine (String.Format("Building SimVsOppLinearModel_SpecificCategory chart '{0}'...", reportPath));

			IEnumerable<CAT_SM_SCN1_SCN2_Tuple> query = _tuples.Values.Where(t => t.Category==_category && t.SimilarityMeasure ==_measure ).OrderBy( t => t.OppositePercentageOtherScenario);
			...
			_log.WriteLine (String.Format("SimVsOppLinearModel_SpecificCategory chart '{0}' is done!", reportPath));
			return reportPath;
		}
		...
		private String BuilLinearModelDataFile(IEnumerable<CAT_SM_SCN1_SCN2_Tuple> query, double R, double slope, double intercept)
		{
			...
			String path = string.Format ("{0}{1}.{2}.LM.dat", _prms.WorkingDir, _category, _measure);
		...
		private String BuildDataFile()
		{
			...
			String path = string.Format ("{0}.{1}.{2}.dat", _prms.WorkingDir, _category, _measure);
		...
		{
			String reportTemplatePath = String.Format ("{0}{1}_{2}.LM.template", _prms.WorkingDir, _category, _measure);
